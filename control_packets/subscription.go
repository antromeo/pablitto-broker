...
func (t *Topic) remove_connection(connection net.Conn) {
	...
	if indice != -1 {
		t.connections = append(t.connections[:indice], t.connections[indice+1:]...)
	}
}
	...
	for _, value := range s.Topics {
		if s.MatchTopic(topic_name, value.topic_name) {
			for _, conn := range value.connections {
				err = s.send(conn, publish_packet)
			}
		}
...
func (s *Subscriptions) MatchTopic(topic_without_wildcard string, topic_optional_wildcard string) bool {
	...
	if wildcard == false {
		if topic_without_wildcard == topic_optional_wildcard {
			return true
		}
	} else {
...
func (s *Subscriptions) compareArrayString(members_1 []string, members_2 []string) bool {
	...
	var compare int
	if len_members_1 > len_members_2 {
		compare = 1
	}
	if len_members_1 == len_members_2 {
		compare = 0
	...
	case 0:
		for index, _ := range members_1 {
			if members_2[index] == "#" {
				...
				if members_2[index] != "+" {
					if members_1[index] != members_2[index] {
						return false
					}
				}
			}
